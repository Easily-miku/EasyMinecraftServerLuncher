...
def main_menu():
    ...
    print(title_big)
    print(title_small)
    print(Fore.GREEN + '[1]启动服务器')
    print(Fore.GREEN + '[2]配置服务器')
    print(Fore.GREEN + '[3]服务器文件')
    print(Fore.GREEN + '[4]信息')
    print(Fore.GREEN + '[5]翻译')
    print(Fore.GREEN + '[6]下载Minecraft服务端')
    print(Fore.GREEN + '[0]关闭软件')
    print(Fore.CYAN + '==========================================')
    shuru = input(Fore.WHITE + '输入对应数字以继续：')
    return shuru
...
def configure_server():
    ...
    if not server_name:
        print(Fore.RED + '未找到服务器文件，请提供服务器目录:')
        server_dir = input('请输入服务器目录路径：')
        if not os.path.isdir(server_dir):
            print(Fore.RED + '目录不存在，请检查路径后重试。')
            input('按回车键返回菜单...')
            return
        server_name = find_server_jar(server_dir)
        if not server_name:
            print(Fore.RED + '提供的目录中仍未找到服务器文件，请检查后重试。')
            input('按回车键返回菜单...')
            return
    else:
        server_dir = '.'  # 当前目录
